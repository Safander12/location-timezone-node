...
writeFileSync(
  `${__dirname}/../src/data/states-ansi.json`,
  JSON.stringify({ data: stringify(statesAnsi) }),
);

// NOTE: write ISO 3611-1 alpha-2 and alpha-3 codes files
writeFileSync(
  `${__dirname}/../src/data/country-iso2-codes.json`,
  JSON.stringify({ data: stringify(countryIso2Codes) }),
);
writeFileSync(
  `${__dirname}/../src/data/country-iso3-codes.json`,
  JSON.stringify({ data: stringify(countryIso3Codes) }),
);

// NOTE: write timezones file
writeFileSync(
  `${__dirname}/../src/data/timezones.json`,
  JSON.stringify({ data: stringify(timezones) }),
);

// NOTE: check every countries iso2 are listed in files
const countryIso2CodesLocations = [
  ...new Set(locationsJson.map((location) => location.countryIso2)),
];
...
const countryIso3CodesLocations = [
  ...new Set(locationsJson.map((location) => location.countryIso3)),
];
...
Object.keys(countries).forEach((country) => {
  countries[country].timezones = [...countries[country].timezones.values()].sort(
